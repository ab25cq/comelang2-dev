                ...
                if(lambda_type.mParamTypes[i-1].mHeap && come_value.type.mHeap) 
                {
                    if(come_value.var) {
                        if(come_value.var.mType.mDelegate) {
                            come_value.var->mCValueName = null;
                        }
                        else if(come_value.var.mType.mShare) {
                            come_value.c_value = increment_ref_count_object(come_value.type, come_value.c_value, info);
                        }
                        else if(come_value.var.mType.mClone) {
                            come_value.c_value = increment_ref_count_object(come_value.type, come_value.c_value, info);
                        }
                        else {
                            come_value.c_value = increment_ref_count_object(come_value.type, come_value.c_value, info);
/*
                            err_msg(info, "require delegate, share, clone for local variable");
                            return false;
*/
                        }
                    }
                    else {
                        if(come_value.type.mDelegate) {
                        }
                        else {
                            come_value.c_value = increment_ref_count_object(come_value.type, come_value.c_value, info);
                        }
                ...
                if(param_types[i].mHeap && obj_value.type.mHeap) {
                    if(obj_value.var) {
                        if(obj_value.var.mType.mDelegate) {
                            obj_value.var->mCValueName = null;
                        }
                        else if(obj_value.var.mType.mShare) {
                            obj_value.c_value = increment_ref_count_object(obj_value.type, obj_value.c_value, info);
                        }
                        else if(obj_value.var.mType.mClone) {
                            obj_value.c_value = increment_ref_count_object(obj_value.type, obj_value.c_value, info);
                        }
                        else {
                            obj_value.c_value = increment_ref_count_object(obj_value.type, obj_value.c_value, info);
